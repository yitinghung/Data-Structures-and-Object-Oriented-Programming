...
void BitToInt(ofstream &outPut, char* HTstr, long long len) {
    int k = 0;
    if (len % nBits != 0) {
...
void CompressFile(const char *file, const char *outPutFile, int num_Node){
    ifstream readIn;
...
int main(int argc, char *argv[]) {
    ...
    CompressFile(argv[1], File_decompressed, num_Node);
    readIn.close();
}
