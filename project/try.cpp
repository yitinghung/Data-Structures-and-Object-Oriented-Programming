...
int main() {
    ...
    char filePath[FILE_LENGTH] = "test.txt";   //"Aesop_Fables.txt"; "graph.txt";  "1.txt";
    char compressFilePath[FILE_LENGTH] = "test2.txt";
...
int BuildHuffmanTree(){
    ...
    for (int i = 0; i < numberOfNode; i++) {
        ...
        reverse(HuffmanCode[ch], HuffmanCode[ch]+k);
        /*
        for (int j = 0; j < k / 2; j++) {
...
void CompressFile(const char *filePath, const char *outPutFilePath, int numberOfNode){
    ...
    for (int i = 0; i < KIND_OF_CHARACTER; i++) {
        if (node[i].number != 0) {
            fprintf(fout, "%d %lld %s\n", i, node[i].number, HuffmanCode[i]);
        }
    }
    
    
    char ch;
    int len=7, temp=0, sum=0;
    
    while((ch=fgetc(fin)) != EOF){
        for(int i=0; i<strlen(HuffmanCode[ch]); i++){
            if(len<0){
                fprintf(fout, "%c", temp);
                printf("!!%c\n", temp);
                len=7;
            ...
            temp+=(HuffmanCode[ch][i]-48)*pow(2, len--);
            
        }
        fprintf(fout, "%s", HuffmanCode[ch]);
    }
    if(temp!=0){
        //cout<<"add:"<<len+1<<endl;
        printf("!!%c\n", temp);
